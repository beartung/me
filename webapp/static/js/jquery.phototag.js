...
(function($) {
	$.fn.photoTag = function( options ){
		...
		var defaultOptions = {
			...
			parametersForRequest : ['image-id','album-id', 'image-link'],
			literals:{
		...
		var createNewTagForm = function( tempTagBox, image, image_id ){
			...
			var submit = $('<input class="inputSubmit btn btn-primary" type="submit" value="' + options.literals.saveTag + '" />');
			...
			var cancel = $('<input class="inputCancel btn" type="button" value="' + options.literals.cancelTag + '"/>');
			...
			$('#tempNewTagForm').submit(function(e){
                ...
                if ($("#tempInput_name")[0].value.length > 0){
		...
		var createTagBoxFromJSON = function( tagJSON, image ){
			...
			innerElement.append('<a href="' + tagJSON.url + '">' + tagJSON.text + '</a>');
			tagBox.append(innerElement);
			if(options.isEnabledToEditTags && tagJSON.isDeleteEnable && options.tag.showDeleteLinkOnTag){
				var deleteLink = $('<a id="'+ options.tag.deleteLinkIdPrefix + tagJSON.id +'" class="'+ options.tag.deleteLinkCssClass +'" href="#'+ tagJSON.id +'">x</a>');
		...
		var createTagItemForList = function( tagJSON, image ){
			var item = $('<li></li>');
			if(tagJSON.url){
				var link = $('<a id="photoTag-tag_'+tagJSON.id+'_link" href="'+ tagJSON.url +'">'+ tagJSON.text +'</a>');
				item.append(link);
			}else{
		...
		var wrapImage = function( image, image_id ){
			...
			var wrapper = $('<div class="' + options.imageWrapBox.cssClass + '" id="' + options.imageWrapBox.idPrefix + image_id +'" style="position:absolute;top:0;left:0;height:'+ imageHeight +'px;width:'+ imageWidth +'px;"><a href="' + image.attr('data-image-link') + '"></a></div>');
